...
public class NearpayPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    // AuthenticationData authentication = authType.equals("userenter") ?
    ...
    // boolean isAuthValidate = authType.equals("userenter") ? true : inputValue ==
    // "" ? false : true;
    ...
    // Map<String, Object> paramMap =
    // NearpayLib.commonResponse(ErrorStatus.already_installed_code,
    // "Plugin Application Already Installed");
    // sendResponse(paramMap);
    // } else if (setupFailure instanceof SetupFailure.NotInstalled) {
    // // when the installtion failed .
    // Map<String, Object> paramMap =
    // NearpayLib.commonResponse(ErrorStatus.not_installed_code,
    // "Plugin Application Installation Failed");
    // sendResponse(paramMap);
    // } else if (setupFailure instanceof SetupFailure.AuthenticationFailed) {
    // // when the Authentication Failed.
    // String messageResp = ((SetupFailure.AuthenticationFailed)
    // setupFailure).toString();
    // String message = messageResp != "" && messageResp.length() > 0 ? messageResp
    // : ErrorStatus.authentication_failed_message;

    // if (authType.equalsIgnoreCase(jwtKey)) {
    // provider.getNearpayLib().nearpay.updateAuthentication(getAuthType(authType,
    // inputValue));
    // Map<String, Object> paramMap =
    // NearpayLib.commonResponse(ErrorStatus.auth_failed_code, message);
    // sendResponse(paramMap);
    // } else {
    // Map<String, Object> paramMap =
    // NearpayLib.commonResponse(ErrorStatus.auth_failed_code, message);
    ...
    // : ErrorStatus.invalid_status_messsage;
    // Map<String, Object> paramMap =
    // NearpayLib.commonResponse(ErrorStatus.invalid_code, message);
    // sendResponse(paramMap);
    // }
    // }
    // });
    ...
    // private void setSession(String sessionID, Boolean enableReceiptUi, Boolean
    // enableReversal, Long finishTimeOut) {
    // provider.getNearpayLib().nearpay.session(sessionID, enableReceiptUi,
    // enableReversal, finishTimeOut,
    // new SessionListener() {
    // @Override
    ...
    // // when the authentication is failed
    // String messageResp = ((SessionFailure.AuthenticationFailed)
    ...
    // // you can get the status using the following code
    // String messageResp = ((SessionFailure.InvalidStatus)
    // sessionFailure).toString();
    // String message = messageResp != "" && messageResp.length() > 0 ? messageResp
    // : ErrorStatus.invalid_status_messsage;
    // Map<String, Object> paramMap =
    ...
    // private static Map<String, Object> sessionResponse(Session session, String
    // message) {
    ...
    // transaction.put("amount_authorized",
    // session.getTransaction().getAmount_authorized());
    // transaction.put("currentcy_code",
    // session.getTransaction().getTransaction_currency_code());
    // transaction.put("cardholder_verification_result",
    // session.getTransaction().getCardholder_verification_result());
    ...
    // transaction.put("transaction_type",
    // session.getTransaction().getTransaction_type());
    // transaction.put("card_scheme_id",
    // session.getTransaction().getCard_scheme_id());
    // transaction.put("system_trace_audit_number",
    // session.getTransaction().getSystem_trace_audit_number());
    ...
    // transaction.put("customer_reference_number",
    // session.getTransaction().getCustomer_reference_number());
    ...
    // List<Map<String, Object>> transactionList = new ArrayList<>();
    // for (TransactionReceipt transReceipt :
    // session.getTransaction().getReceipts()) {
    // String jsonStr = ReceiptUtilsKt.toJson(transReceipt);
    // transactionList.add(NearpayLib.JSONStringToMap(jsonStr));
    // }
    ...
    // // Card Scheme
    // LocalizationField cardSchemeTrans =
    // session.getTransaction().getCard_scheme();
    ...
    // // Card Scheme
    // LocalizationField versificatioType =
    // session.getTransaction().getVerification_method();
    ...
    // private void getRecieptImageTrans(TransactionReceipt recipt, int imgwidth,
    // int fontSize) {
    // ReceiptUtilsKt.toImage(recipt, this.provider.getNearpayLib().context,
    // imgwidth, fontSize,
    ...
    // private void getRecieptImageReconcile(ReconciliationReceipt recipt, int
    // imgwidth, int fontSize) {
    // ReceiptUtilsKt.toImage(recipt,
    // this.provider.getNearpayLib().context, imgwidth, fontSize, (BitmapListener)
    // bitmap -> {
    // // bitmap
